...
{
    for (auto& b : bindings) {
        b.stageFlags |= shaderStages; //Add all stage flags
    }
...
/*void DescriptorAllocator::init_pool(VkDevice device, uint32_t maxSets, std::span<PoolSizeRatio> poolRatios)
{
    ...
    // alongside a ratio to multiply the maxSets parameter is. This lets us directly control how big the pool is going to be
...
{
    vkResetDescriptorPool(device, pool, 0); //Reset does not actually delete the pool but it does bring it back to an initial state
}
