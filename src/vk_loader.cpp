...
std::optional<std::vector<std::shared_ptr<MeshAsset>>> loadGltfMeshes(VulkanEngine* engine, std::filesystem::path filePath)
{
    ...
    for (fastgltf::Mesh& mesh : gltf.meshes) {
        ...
        constexpr bool OverrideColors = false; //Override the color with normal values
        if (OverrideColors) {
...
std::optional<std::shared_ptr<LoadedGLTF>> loadGltf(VulkanEngine* engine, std::string_view filePath)
{
    ...
    //Now we have to load everything in order. MeshNodes depend on meshes, meshes depend on materials, and materials on textures
    // load all textures
    for (fastgltf::Image& image : gltf.images) {
        ...
        if (img.has_value()) {
            ...
            // we failed to load, so lets give the slot a default checkerboard texture to not
    ...
    for (fastgltf::Mesh& mesh : gltf.meshes) {
        ...
        for (auto&& p : mesh.primitives) {
            ...
            //loop the vertices of this surface, find min/max bounds (useful for frustum culling and other algos)
...
void LoadedGLTF::Draw(const glm::mat4& topMatrix, DrawContext& ctx)
{
    ...
    for (auto& n : topNodes) {
        //Calling draw on top nodes will recursively loop through and update all children
